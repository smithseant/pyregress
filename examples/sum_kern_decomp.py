...
from numpy import empty, linspace, expand_dims, maximum, abs, sqrt
...
source_gp = GPI(empty((0, 1)), empty(0), Kt)
Yd = source_gp.sample(Xd)

# Calculate the posterior GPs based on each of the individual kernels
Ni = 200
Xi = expand_dims(linspace(-20.0, 70.0, Ni), 1)
my_gp = GPI(Xd, Yd, Kt)
meanYt_i, stdYt_i = my_gp(Xi, infer_std=True)
meanYa_i, stdYa_i = my_gp(Xi, infer_std=True, kernel_terms=0)
meanYb_i, stdYb_i = my_gp(Xi, infer_std=True, kernel_terms=1)
