# -*- coding: utf-8 -*-
"""
! Please note that either pyDOE or DOE_spacefilling is required to run this validation case

Validation case 'Borehole' from
H. Chen et al., "Analysis methods for computer experiments: How to assess
    and what counts?" Submitted to Statistical Science June 29, 2014.

Created on Tue Nov 11, 2014 @author: ben
"""

from numpy import empty, ones, abs, max, sqrt, log, pi as π
from numpy.random import permutation
# from pyDOE import lhs
from DOE_spacefilling import optmaximin_naive
from pyregress import GPI, SquareExp, Jeffreys

def Borehole(mean, samples=27, permutations=1, errorType='RMSE'):
    '''
    Borehole( mean, samples, permutations, errorType)
    ...
    def borehole_function(x):
        ...
        numerator   = 2 * π * Tu_ * (Hu_ - Hl_)
        denomenator = (log(r_ / rw_) *
                       (1 + 2 * L_ * Tu_ / (log(r_ / rw_) * rw_**2 * Kw_) +
                        Tu_ / Tl_))
    ...
    output_test = empty(samples_test)
    ...
    for p in range(permutations):
        ...
        if mean == 'constant':
            mean = [0]  # specified as const. across all x
        if mean == 'fl':
            mean = [0, 1] # specified as const. + linear for all x
        myKernel = SquareExp(w=Jeffreys(guess=100),
                             l=[Jeffreys(guess=2),
                                ...
                                Jeffreys(guess=2),
                                Jeffreys(guess=2)])
        myGP = GPI(lhd, output, myKernel, explicit_basis=mean)
...
if __name__ == "__main__":
    ...
    permutations = 25 # permutations of lhd columns
    ...
    print('{} Const. Mean - '.format(error_type))
    print(errors_const)
    print('{} Full Linear Mean - '.format(error_type))
    print(errors_fl)

    # Plot
    fig = plt.figure(figsize=(6, 3))
    plt.plot(1 * ones(permutations), errors_const, 'o')
    plt.plot(2 * ones(permutations), errors_fl, 'o')
    ...
    plt.title('{}i-run mLHD with Gauss Kernel'.format(samples), fontsize=12)
    plt.show()
