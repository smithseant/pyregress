...
class BaseTransform(metaclass=ABCMeta):
    ...
    def __init__(self, *args, **kwargs):
        ...
        pass
    def __call__(self, x, inverse=False, grad_y=None, grad_z=None, hess_z=None):
        """Perform the transformation or inverse transformation."""
        if not inverse:
            return self.transform(x, grad_y=grad_y)
        else:
            return self.inv_transform(x, grad_z=grad_z, hess_z=hess_z)

    @abstractmethod
    def transform(self, y, grad_y=None):
        """
        ...
        grad_y: array-2D (optional),
            gradient of the dependent variables with respect to its own independent variables.

        Returns
        -------
        z:  array (same shape as y),
            transformed dependent variable values.
        grad_z:  array (optional, depending on the argument grad_z),
            transformed gradient only returned when grad_y is provided.
        """
        return
    @abstractmethod
...
class Logarithm(BaseTransform):
    ...
    def transform(self, y, grad_y=None):
        if grad_y is None:
            ...
            return log(y), grad_y / y.reshape((-1, 1))
    def inv_transform(self, z, grad_z=None, hess_z=None):
        y = exp(z)
        if grad_z is None:
            return y
...
class Logit(BaseTransform):
    ...
    def transform(self, y, grad_y=None):
        if grad_y is None:
            ...
            return log(y/(1 - y)), ((2 - y) / (y * (1 - y))).reshape((-1, 1)) * grad_y
    def inv_transform(self, z, grad_z=None, hess_z=None):
        y = 1 / (exp(-z) + 1)
        if grad_z is None:
            return y
