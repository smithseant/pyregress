...
samples = 7
points = 30
Xs =  sort(2.0*(random(points)).reshape((-1,1)),axis=0)
...
sourceGP1 = GPR(zeros((0,0)), zeros(0), myK)
Ys = zeros((samples,points))
for i in xrange(samples):
    Ys[i,:] = sourceGP1.sample(Xs).reshape(points)
    plt.plot(Xs,Ys[i,:],'.-',markersize=6)
plt.title('Squared Exp - w=2.0, l=0.25',fontsize=10)

plt.subplot(2,2,2)
myK = SquareExp(w=0.5, l=1.0) 
sourceGP2 = GPR(zeros((0,0)), zeros(0), myK)
Ys = zeros((samples,points))
for i in xrange(samples):
    Ys[i,:] = sourceGP2.sample(Xs).reshape(points)
    plt.plot(Xs,Ys[i,:],'.-',markersize=6)
...
fig1.text(.45,.01,'Sample Points',fontsize=16)
fig1.text(.35,.95,'Random Samples from GP',fontsize=20)
